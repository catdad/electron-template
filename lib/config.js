...
const directory = process.env[`${pkg.name.toUpperCase().replace(/-/g, '_')}_CONFIG_PATH`] || path.resolve(is.prod ? is.appData : root);
const location = path.resolve(directory, `.${pkg.name}-config.json`);
